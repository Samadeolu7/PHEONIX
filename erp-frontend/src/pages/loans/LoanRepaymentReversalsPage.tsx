/**
 * Approvals inbox for loan repayment reversals — undoing a specific,
 * already-posted loan payment. Always requires two different approvers: the
 * first approves, then a second, different approver confirms and that
 * second approval is what actually executes the reversal (GL entry +
 * schedule + balances). Neither the requester nor the first approver can
 * act as the second approver.
 *
 * Route: /loans/repayment-reversals
 */
...
import { LoanRepaymentReversal, RepaymentReversalStatus } from '../../services/loanService';
import { useAuth } from '../../contexts/AuthContext';
import {
  useLoanRepaymentReversals,
  useFirstApproveRepaymentReversal,
  useSecondApproveRepaymentReversal,
  useRejectRepaymentReversal,
} from '../../hooks/useLoans';
...
const STATUS_BADGE: Record<RepaymentReversalStatus, string> = {
...
const STATUS_LABEL: Record<RepaymentReversalStatus, string> = {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <label className="mb-1 block text-xs font-medium text-gray-600" htmlFor="rev-notes">
...
interface RowProps {
  req: LoanRepaymentReversal;
  currentUserId: number | undefined;
  onFirstApprove: (req: LoanRepaymentReversal) => void;
  onSecondApprove: (req: LoanRepaymentReversal) => void;
  onReject: (req: LoanRepaymentReversal) => void;
}
  ...
  return (
    <tr className="hover:bg-gray-50 align-top">
      ...
      <td className="px-4 py-3">
        ...
        <div className="text-xs text-gray-400">{req.loan_number}</div>
        <div className="text-xs text-gray-400 font-mono">{req.journal_entry_reference}</div>
      </td>
      <td className="px-4 py-3 text-sm text-gray-900">₦{fmt(req.amount)}</td>
      ...
      <td className="px-4 py-3 text-right">
        {req.status === 'pending' && (
          isRequester ? (
            <span className="text-xs text-gray-400" title="Maker-checker: you requested this reversal">
        ...
        {req.status === 'awaiting_second_approval' && (
          isRequester || isFirstApprover ? (
            <span className="text-xs text-gray-400" title="Maker-checker: a different approver must give the second approval">
...
type StatusFilter = RepaymentReversalStatus | 'all';

export default function LoanRepaymentReversalsPage() {
  const { user } = useAuth();
  const [filter, setFilter] = useState<StatusFilter>('pending');
  const [modal, setModal] = useState<{ req: LoanRepaymentReversal; kind: 'first' | 'second' | 'reject' } | null>(null);

  const { data: requests = [], isLoading: loading, error: queryError, refetch } = useLoanRepaymentReversals(
    filter === 'all' ? {} : { status: filter }
  );
  const firstApproveMutation = useFirstApproveRepaymentReversal();
  const secondApproveMutation = useSecondApproveRepaymentReversal();
  const rejectMutation = useRejectRepaymentReversal();

  const error = queryError ? (queryError as any)?.response?.data?.error ?? queryError.message ?? 'Failed to load reversals.' : null;
  ...
  const actionError = (firstApproveMutation.error as any)?.response?.data?.error
    || firstApproveMutation.error?.message
    || (secondApproveMutation.error as any)?.response?.data?.error
    || secondApproveMutation.error?.message
    || (rejectMutation.error as any)?.response?.data?.error
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-gray-900">Repayment Reversals</h1>
              {pendingCount > 0 && (
            ...
            <p className="text-sm text-gray-500">
              Undo a specific, already-posted loan payment — always requires two different approvers.
            </p>
          </div>
      ...
      <div className="mx-auto max-w-6xl p-6">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-8 text-center">
            ...
            <p className="text-sm text-gray-500">No {filter === 'all' ? '' : STATUS_LABEL[filter as RepaymentReversalStatus]?.toLowerCase() ?? ''} reversals.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead className="border-b border-gray-200 bg-gray-50 text-xs font-semibold uppercase text-gray-500">
                <tr>
                  ...
                  <th className="px-4 py-3">Loan / Payment</th>
      ...
      {modal && modal.kind === 'first' && (
        <NotesModal
          ...
          description={`Reverse a ₦${fmt(modal.req.amount)} payment on ${modal.req.loan_number}? A second, different approver must confirm before anything actually happens.`}
      ...
      {modal && modal.kind === 'second' && (
        <NotesModal
          ...
          description={`This executes immediately: the ₦${fmt(modal.req.amount)} payment on ${modal.req.loan_number} is reversed on the GL and its schedule/balances are unwound. This cannot be undone through this workflow.`}
      ...
      {modal && modal.kind === 'reject' && (
        <NotesModal
          ...
          description="The original payment is left exactly as it is."
          confirmLabel="Reject"
          confirmClassName="bg-red-600 hover:bg-red-700"
          loading={actionLoading}
          onClose={() => setModal(null)}
