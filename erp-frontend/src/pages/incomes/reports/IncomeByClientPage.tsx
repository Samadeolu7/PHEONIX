...
// Top clients ranked by revenue — invoiced, collected, outstanding
...
const IncomeByClientPage: React.FC = () => {
  ...
  const [data,     setData]     = useState<IncomeByClientData | null>(null);
  ...
  const load = useCallback(async (
    ...
    try {
      setData(
        await incomeReportsService.getByClient({ date_from: from, date_to: to, sort, limit: lim })
      );
    } catch (e: any) {
  ...
  useEffect(() => { load(dateFrom, dateTo, sortBy, limit); }, []);

  const handleApply = () => load(dateFrom, dateTo, sortBy, limit);

  const handleSort = (field: ClientSortField) => {
    setSortBy(field);
    load(dateFrom, dateTo, field, limit);
  };
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        {!loading && data && filteredRows.length === 0 && (
          <div className="bg-white border border-gray-200 rounded-xl py-16 text-center text-gray-400 text-sm">
            {search ? 'No clients match your search.' : 'No income data found for the selected period.'}
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <SortHeader field="invoiced"    label="Invoiced (₦)"    current={sortBy} onSort={handleSort} />
                  <SortHeader field="collected"   label="Collected (₦)"   current={sortBy} onSort={handleSort} />
                  <SortHeader field="outstanding" label="Outstanding (₦)" current={sortBy} onSort={handleSort} />
                  <th className="text-left px-5 py-3 font-medium text-gray-600 min-w-[140px]">Collection</th>
                </tr>
              </thead>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          {/* Revenue bar relative to top client */}
