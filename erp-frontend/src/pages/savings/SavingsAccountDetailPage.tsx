  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p className="mb-4 text-sm text-gray-500">{account.account_number} — {account.client_name}</p>
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button type="submit" disabled={submitting} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50">
                {submitting ? <Loader2 size={14} className="animate-spin" /> : <ArrowDownCircle size={14} />}
                Post Deposit
              </button>
            </div>
          </form>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p className="mb-2 text-sm text-gray-500">{account.account_number} — {account.client_name}</p>
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button type="submit" disabled={submitting} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-orange-600 px-4 py-2 text-sm font-medium text-white hover:bg-orange-700 disabled:opacity-50">
    ...
    <div className="min-h-screen bg-gray-50">
      {showDeposit && (
        <DepositModal
          ...
          onSuccess={() => { setShowDeposit(false); loadAccount(); loadTransactions(); }}
        />
      )}
      {showWithdraw && (
        <WithdrawModal
          ...
          onSuccess={() => { setShowWithdraw(false); loadAccount(); loadTransactions(); }}
        />
      )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${STATUS_BADGE[account.status] ?? 'bg-gray-100 text-gray-600'}`}>
                  {account.status}
                </span>
              </div>
              <p className="text-sm text-gray-500">{account.client_name} — {account.product_name}</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <button onClick={() => { loadAccount(); loadTransactions(); }} className="rounded-lg border border-gray-300 p-2 text-gray-500 hover:bg-gray-50" title="Refresh">
              <RefreshCw size={16} />
            </button>
      ...
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: 'Current Balance', value: `₦${fmt(account.current_balance)}`, color: 'text-gray-900' },
            { label: 'Available Balance', value: `₦${fmt(account.available_balance)}`, color: 'text-green-700' },
            { label: 'Interest Rate', value: `${account.interest_rate}% p.a.`, color: 'text-gray-700' },
        ...
        <div className="rounded-xl bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wide text-gray-500">Account Details</h2>
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-3 lg:grid-cols-4">
            ...
            <div className="col-span-2">
              <Link to={`/clients/${account.client}`} className="text-sm text-blue-600 hover:underline">
                View Client Profile →
              </Link>
            </div>
          </div>
            ...
            {schedule.length === 0 && !scheduleLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {schedule.map((row) => (
                      <tr key={row.id} className={row.status === 'missed' ? 'bg-red-50' : 'hover:bg-gray-50'}>
                        <td className="px-4 py-2.5 text-gray-700">{fmtDate(row.expected_date)}</td>
                        <td className="px-4 py-2.5 text-right font-medium text-gray-900">₦{fmt(row.expected_amount)}</td>
                        <td className="px-4 py-2.5">
                          <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${CONTRIB_STATUS_BADGE[row.status] ?? 'bg-gray-100 text-gray-600'}`}>
                            {row.status}
                          </span>
                        </td>
